...
def getValueFromREDCap( pGUID, event, measure ):
    ...
    data = {
        'token': '<REDCap Token required to connect>',
        'content': 'record',
        'format': 'json',
        'type': 'flat',
        'records[0]': pGUID,
    ...
    ch = pycurl.Curl()
    ch.setopt(ch.URL, 'https://<machine running REDCap>/redcap/api/')
...
if __name__ == "__main__":
        ...
        if eprime.shape[0] == 0:
                ...
                sys.log('Error: could not read e-prime file as csv')
                sys.exit(0)
        ...
        if 1:
                ...
                if mode == "file":
                    ...
                    if 'ABCD-SST-fMRI' in anonInfo['ClassifyType'] or 'ABCD-MID-fMRI' in anonInfo['ClassifyType'] or 'ABCD-nBack-fMRI' in anonInfo['ClassifyType']: 
                        eprime_flags = lookupEdat2(anonInfo['SeriesInstanceUID']) 
                        if eprime_flags == -1:
